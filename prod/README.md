...
| 7 | Root Application | App of Apps (303 레포의 prod/root 참조) |
...
```bash
cd 302-goormgb-k8s-bootstrap/prod
make install-all
```
...
export DB_HOST=$(aws secretsmanager get-secret-value \
  --secret-id prod/services/db --query 'SecretString' --output text | jq -r '.host')
export DB_PASSWORD=$(aws secretsmanager get-secret-value \
  --secret-id prod/services/db --query 'SecretString' --output text | jq -r '.password')
export DB_USER=$(aws secretsmanager get-secret-value \
  --secret-id prod/services/db --query 'SecretString' --output text | jq -r '.username')
export DB_NAME=$(aws secretsmanager get-secret-value \
  --secret-id prod/services/db --query 'SecretString' --output text | jq -r '.dbname')
...
```bash
cd ~/01_KDT/301-goormgb-terraform/environments/prod
...
```bash
cd ~/01_KDT/302-goormgb-k8s-bootstrap/prod
make install-all
```
...
```bash
# ArgoCD UI에서 sync 또는
argocd app sync root-prod --prune
```
