#!/bin/bash
# ============================================================
# staging/ai/ai-init.sh
# AI Defense PostgreSQL 초기화 스크립트 (Staging)
#
# 사용법:
#   ./ai-init.sh
#
...
#   DB_USER     - 데이터베이스 사용자 (기본: ai_defense)
...
echo -e "${BLUE}============================================================${NC}"
echo -e "${BLUE}  AI Defense DB 초기화 (Staging)${NC}"
echo -e "${BLUE}============================================================${NC}"
echo ""
...
if PGPASSWORD="$DB_PASSWORD" psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "SELECT 1" > /dev/null 2>&1; then
    ...
    echo "  1. ai_defense 데이터베이스가 생성되어 있는지 확인"
    echo "  2. RDS 보안그룹 확인"
    echo "  3. Bastion 또는 port-forward 확인"
    exit 1
fi
echo ""
