...
make 00-install-all    # 1. 전체 설치 (ESO → Karpenter → ArgoCD → Root App)
make 01-db-init        # 2. PostgreSQL 초기화
make 02-ai-init        # 3. AI Defense DB 초기화
```

CA 환경(`ca-staging`, `ca-prod`)은 Bastion SSM 포트포워딩 + Secrets Manager 자동 조회.
...
### staging/prod, ca-staging/ca-prod (EKS)
